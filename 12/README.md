...
* Generated by a process that takes one or more datasets as inputs (e.g. training a model on 2 datasets) and produces a new data asset.
...
* Entity-Activity interactions 
  ...
  - Activities make use of Entities.  (used)
    - Example: A model training Activity used a training dataset Entity
...
|-----------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| User Bob uploaded a dataset to Ocean                | (Agent) Bob was associated with a publishing activity, which generated a (Entity) data asset                                                                                                                            |
| Bob invoked a Consent operation to filter a dataset | (Agent) Bob was associated with a filtering activity, which *used* an (Entity) data asset and  generated a new (Entity) data asset. The consent service (Entity) was involved in (associated with) generating the asset |
|                                                     |                                                                                                                                                                                                                         |
...
Ocean adds the following additional *types* for entities, activities and agents. 

* Entities:
  - op:dataset : An Ocean dataset asset 
  - op:operation : Ocean service or operation 

* Activities:

  - op:publish : The act of publishing a data asset
  - op:import : The act of importing a data asset
  - op:invoke_operations : The act of invoking an operation.

* Agents:

  - op:did : DID of an Ocean Agent, if available.
  - op:id_type : The type of the agent id, could be `ethereum_account` or `did` 
...
* The metadata must declare the following nodes in the prov graph:
  ...
  - Activity (publishing)
  - Agent (the ID of the person, organization or software) that created the dataset
...
* The Agent involved, could be a prov:Person, prov:Organisation or prov:SoftwareAgent
 ...
 - An "op:did" to indicate the DID if the user has a DID, else the ethereum account. 
   - the "op:id_type" should be `DID` in the first case and `ethereum_account` in the second case.
```
"agent": {
    "op:op_abvfwi89sjek": {
      ...
      "op:did": "did:op:26cb1a92e8a6b52e47e6e13d04221e9b005f70019e21c4586dad3810d46220135/32561a92e8a6b52e47e6e13d04221e9b005f70019e21c4586dad3810d46220135",
      "op:id_type":"did"
    }
  }
```
...
* The Activity (publish) 
 - _type_ indicates the type of activity. (e.g. `op:publish` is one of the supported Activity types)
...
```
  "activity": {
    "op:op_asf29fj291lkd": {
      ...
      "prov:type": "op:publish"
    }
  }
```

* The Entity (dataset) that was published
  - An "prov:type" attribute that could take values `op:dataset` or `op:operation`.
...
"entity": {
    "op:this": {
      ...
      "prov:type":"op:dataset",
      "op:did": "did:op:26cb1a92e8a6b52e47e6e13d04221e9b005f70019e21c4586dad3810d46220135/503a7b959f91ac691a0881ee724635427ea5f3862aa105040e30a0fee50cc1a00"
    }
  }
```

* Generation attribute (connecting Entity with Activity)
...
* Association attribute (connecting Agent with Activity)
...
- It could use one or more datasets or algorithms as input  (e.g a trained model would need an algorithm, and a training set as inputs)
...
- Association (wasAssociatedWith)
  - the Agent involved in the activity
- Used (used)
...
"agent": {
    ...
    "op:op_abv2837282isk": {
      ...
      "op:did": "did:op:27cb1a92e8a6b52e47e6e13d04221e9b005f70019e21c4586dad3810d46220135/513a7b959f91ac691a0881ee724635427ea5f3862aa105040e30a0fee50cc1a00"
    }
  }
```
...
"entity": {
    "op:this": {
      ...
      "prov:type":"op:dataset",
      "op:did": "did:op:17cb1a92e8a6b52e47e6e13d04221e9b005f70019e21c4586dad3810d46220135/113a7b959f91ac691a0881ee724635427ea5f3862aa105040e30a0fee50cc1a00"
    }
  }
```
...
"wasAssociatedWith": {
    ...
    "_:wAW5": {
      "prov:agent": "op:op_abv2837282isk",
      "prov:activity": "op:op_asf29fj291lkd"
    }
  }
```
...
4. Which entity generates the provenance metadata?
 ...
 - When a user creates a derived dataset (e.g by cleaning a raw dataset), then they may manually add provenance metadata.
