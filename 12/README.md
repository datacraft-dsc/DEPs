...
contributors: Mike Anderson <mike.anderson@datacraft.sg>, Kiran Karkera <kiran.karkera@dex.sg>, Bill Barman <bill.barman@dex.sg>
```
...
=================
   ...
   * [Metadata recommendations by use case](#examples-of-provenance-metadata)
   * [FAQ](#faq)
   * [License](#license)
...
This document defines the DEP standard approach for a publisher to declare the provenance of assets in
the context of Digital Supply Lines. In this context, data assets could be:

* Published by a person or organisation
* Updated by a different person or organisation
* Generated by transformations of data assets (e.g. by execution of a DEP6 Invoke Operation)
...
A key feature of the DEP12 Provenance standard is the ability to use the provenance to communicate the complete Digital Supply Chain that was was used to generate an asset, which in turn enables Digital Supply Chain opeartions to be reprodicible (assuming input assets are available to the reproducer).

## Motivation and Use cases

Provenance information can be considered from three perspectives:
...
- Finding derivatives of a data asset
  -  e.g an model training algorithm has been involved in generating several assets (e.g. trained models). Let's assume that a data scientist found a bug in the algorithm and fixed it. Provenance metadata can help us find the data assets generated by this algorithm, which need to be updated.
  - The publisher of a data asset A1 requires that all data assets that used A1 must pay licensing fees as a percentage of their selling price. Provenance metadata can help us find all the data assets that used data asset A1.
- Reproducibility
...
The DEP12 Provenance Standard build upon the W3C PROV Standard. Readers may wish to refer to a longer introduction about the W3C Provenance standard [here](https://www.w3.org/TR/2013/NOTE-prov-primer-20130430/#intuitive-overview-of-prov).
...
In the context of the DEP-based data ecosystem,

* Data Assets and Algorithms are Entities. 
* Publishers, Consumers and Service Providers are Agents.
* Publishing and Jobs which represent the invoking of Operations are Activities. 
...
Note that the same asset may be stored on different agents, in which case it may be possible to obtain the same asset from a different source for the purposes of validation or reproducibility.

### Agents 

Including Agents as separate PROV entitiesAgents is not mandatory but recommended since it provides provenance regarding who executed the Digital Supply Chain.

Prov metadata may include two types of agents based on category:

#### Service Providers

Service providers such as Storage service provider, with the DID of the Agent
...
* Entities:
  ...
  - `dep:this`: A special attribute value that refers to the Entity being published or generated. 
  
* Activities:

  - `dep:publish` : The act of publishing a data asset.
  - `dep:import` : The act of importing a data asset which is defined in an existing data catalog. `Import` is used to distinguish from `publish` to indicate that prior provenance metadata associated with the data asset will be incorporated.
  - `dep:invoke` : The act of invoking an operation.
...
# Metadata recommendations by use case

### Use case 1: Publishing an asset

It is recommended that the publisher includes `Prov` metadata when an asset is published.

The prov metadata should include:
...
### Use case 2: Importing data asset 
...
### Use case 3: An operation generating a new asset by transforming an existing asset

Operations such as data cleaning, model training and others, generate new data assets. This process could have the following characteristics
